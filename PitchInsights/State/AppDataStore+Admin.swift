...
extension AppDataStore {
    func seedAdminPlaceholderDataIfNeeded() {
        guard adminPersons.isEmpty else { return }

        let now = Date()
        let seasonStart = Calendar.current.date(from: DateComponents(year: 2025, month: 7, day: 1)) ?? now
        let seasonEnd = Calendar.current.date(from: DateComponents(year: 2026, month: 6, day: 30)) ?? now

        let trainerLead = AdminPerson(
            backendID: "local.admin.coach.head",
            fullName: "Tyler Tenger",
            email: "tyler@pitchinsights.local",
            personType: .trainer,
            role: .chefTrainer,
            teamName: "1. Mannschaft",
            permissions: Set(AdminPermission.allCases),
            presenceStatus: .active,
            isOnline: true,
            linkedMessengerUserID: "coach.tyler",
            lastActiveAt: now
        )
        let trainerCo = AdminPerson(
            backendID: "local.admin.coach.assistant",
            fullName: "Sven Krüger",
            email: "sven@pitchinsights.local",
            personType: .trainer,
            role: .coTrainer,
            teamName: "1. Mannschaft",
            permissions: [.trainingCreate, .trainingEdit, .squadEdit, .manageGroups, .publishTrainingReports],
            presenceStatus: .active,
            isOnline: true,
            linkedMessengerUserID: "coach.sven",
            lastActiveAt: now.addingTimeInterval(-600)
        )
        let analyst = AdminPerson(
            backendID: "local.admin.analyst",
            fullName: "Mara Hoffmann",
            email: "mara@pitchinsights.local",
            personType: .trainer,
            role: .analyst,
            teamName: "1. Mannschaft",
            permissions: [.publishTrainingReports, .manageMessengerRights],
            presenceStatus: .active,
            isOnline: false,
            linkedMessengerUserID: "analyst.mara",
            lastActiveAt: now.addingTimeInterval(-3600)
        )

        let mappedPlayers: [AdminPerson] = players.map { player in
            AdminPerson(
                backendID: "local.admin.player.\(player.id.uuidString.lowercased())",
                fullName: player.name,
                email: "\(player.name.replacingOccurrences(of: " ", with: ".").lowercased())@club.local",
                personType: .player,
                role: nil,
                teamName: player.teamName,
                permissions: [],
                presenceStatus: player.availability == .unavailable ? .away : .active,
                isOnline: player.availability != .unavailable,
                linkedPlayerID: player.id,
                linkedMessengerUserID: "player.\(player.id.uuidString.lowercased())",
                lastActiveAt: now.addingTimeInterval(-Double.random(in: 900...6400))
            )
        }

        let rehabGroup = AdminGroup(
            backendID: "local.admin.group.reha",
            name: "Reha-Gruppe",
            goal: "Belastungssteuerung",
            groupType: .temporary,
            memberIDs: mappedPlayers
                .filter { $0.presenceStatus == .away }
                .map(\.id),
            responsibleCoachID: trainerCo.id,
            assistantCoachID: trainerLead.id,
            startsAt: now,
            endsAt: Calendar.current.date(byAdding: .day, value: 21, to: now),
            createdAt: now,
            updatedAt: now
        )
        let attackGroup = AdminGroup(
            backendID: "local.admin.group.attack",
            name: "Sturm",
            goal: "Abschlussqualität",
            groupType: .permanent,
            memberIDs: mappedPlayers
                .filter { player in
                    guard let linked = player.linkedPlayerID,
                          let source = players.first(where: { $0.id == linked }) else {
                        return false
                    }
                    return source.primaryPosition.group == .attack
                }
                .map(\.id),
            responsibleCoachID: trainerLead.id,
            assistantCoachID: analyst.id,
        ...
        var allPersons = [trainerLead, trainerCo, analyst] + mappedPlayers
        for index in allPersons.indices {
            if rehabGroup.memberIDs.contains(allPersons[index].id) {
                allPersons[index].groupIDs.append(rehabGroup.id)
            }
            if attackGroup.memberIDs.contains(allPersons[index].id) {
                allPersons[index].groupIDs.append(attackGroup.id)
            }
        }

        adminPersons = allPersons
        adminGroups = [attackGroup, rehabGroup]
        adminInvitations = [
            AdminInvitation(
                backendID: "local.admin.invite.1",
                recipientName: "Paul Weber",
                ...
                inviteLink: nil,
                sentBy: "Tyler Tenger",
                sentAt: now.addingTimeInterval(-3600 * 5),
                expiresAt: Calendar.current.date(byAdding: .day, value: 7, to: now) ?? now
            )
        ]
        adminSeasons = [
            AdminSeason(
                backendID: "local.admin.season.2025",
                name: "Saison 2025/26",
                startsAt: seasonStart,
                endsAt: seasonEnd,
                status: .active,
            ...
            AdminSeason(
                backendID: "local.admin.season.2024",
                name: "Saison 2024/25",
                startsAt: Calendar.current.date(byAdding: .year, value: -1, to: seasonStart) ?? seasonStart,
                endsAt: Calendar.current.date(byAdding: .year, value: -1, to: seasonEnd) ?? seasonEnd,
                status: .archived,
                teamCount: 1,
                playerCount: max(18, mappedPlayers.count - 2),
                trainerCount: 2
            )
        ]
        activeAdminSeasonID = adminSeasons.first(where: { $0.status == .active })?.id
        adminClubSettings = .default
        adminMessengerRules = .default
        adminAuditEntries = [
            AdminAuditEntry(
                backendID: "local.admin.audit.1",
                actorName: "Tyler Tenger",
                targetName: "Sven Krüger",
                area: .roles,
                action: "Rechte angepasst",
                details: "Trainingsbericht-Freigabe aktiviert.",
                timestamp: now.addingTimeInterval(-3600 * 2)
            ),
            AdminAuditEntry(
                backendID: "local.admin.audit.2",
                actorName: "Mara Hoffmann",
                targetName: "Reha-Gruppe",
                area: .groups,
                action: "Gruppe aktualisiert",
                details: "Dauer bis Monatsende verlängert.",
                timestamp: now.addingTimeInterval(-3600 * 6)
            )
        ]
        adminConnectionState = AppConfiguration.isPlaceholder ? .placeholder : .live
