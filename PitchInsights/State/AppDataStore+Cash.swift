...
extension AppDataStore {
    ...
    func seedCashPlaceholderDataIfNeeded() {
        ensureCashCategories()
        updateCashAccessContext()
        if !cashTransactions.isEmpty && !cashMonthlyContributions.isEmpty && !cashGoals.isEmpty {
            return
        }
        ...
        let previousMonth = calendar.date(byAdding: .month, value: -1, to: monthStart) ?? now

        let categoryByName = Dictionary(uniqueKeysWithValues: cashCategories.map { ($0.name, $0) })
        let contribution = categoryByName["Beiträge"] ?? cashCategories[0]
        let material = categoryByName["Material"] ?? cashCategories[0]
        let events = categoryByName["Events"] ?? cashCategories[0]
        let drinks = categoryByName["Getränke"] ?? cashCategories[0]
        ...
        if cashTransactions.isEmpty {
            cashTransactions = [
                CashTransaction(
                    ...
                    description: "Monatsbeiträge Team",
                    type: .income,
                    paymentStatus: .paid,
                    syncState: .synced
                ),
                CashTransaction(
                    ...
                    type: .expense,
                    responsibleTrainerID: adminPersons.first.map { $0.backendID ?? $0.id.uuidString },
                    paymentStatus: .paid,
                    syncState: .synced
                ),
                CashTransaction(
                    ...
                    description: "Getränke Heimspiel",
                    type: .expense,
                    responsibleTrainerID: adminPersons.dropFirst().first.map { $0.backendID ?? $0.id.uuidString }
                        ?? adminPersons.first.map { $0.backendID ?? $0.id.uuidString },
                    paymentStatus: .paid,
                    syncState: .synced
                ),
                CashTransaction(
        ...
        if cashMonthlyContributions.isEmpty {
            let dueCurrent = calendar.date(byAdding: .day, value: 7, to: monthStart) ?? now
            let duePrevious = calendar.date(byAdding: .day, value: 7, to: previousMonth) ?? now
            for (index, player) in players.enumerated() {
                cashMonthlyContributions.append(
                    MonthlyContribution(
                        playerID: player.id,
                        amount: 35,
                        dueDate: dueCurrent,
                        status: index.isMultiple(of: 3) ? .open : .paid,
                        monthKey: Self.monthKey(for: monthStart)
                    )
                )
                cashMonthlyContributions.append(
                    MonthlyContribution(
                        ...
                        monthKey: Self.monthKey(for: previousMonth)
                    )
                )
            }
        }

        if cashGoals.isEmpty {
            cashGoals = [
                CashGoal(
                    name: "Neues Trikotset",
                    targetAmount: 2500,
